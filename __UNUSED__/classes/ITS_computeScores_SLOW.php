...
ITS_score - compute user lab scores.
	 ...
	 Last Update: Nov-08-2010
*/	 
//=====================================================================//
class ITS_score {
	
	public function __construct($userid,$status,$ch) {
		...
		//echo $this->userid.' - '.$this->status.' - '.$this->chapter; //die();
		// connect to database
	...
	//=====================================================================//
		...
		$usertable = "stats_".$this->userid;
		$s = new ITS_statistics($this->userid,'Spring_2010',$this->status);
		...
		for($i=0;$i<$ch;$i++) {
			...
			$c = $i + 1;
			//$query1 = "SELECT question_id,qtype FROM ".$usertable.",webct WHERE current_chapter='".$c."' AND question_id=webct.id";
			
			//if ($this->status == 'admin') { 
				$query1 = 'SELECT question_id,qtype FROM '.$usertable.',webct WHERE '.$usertable.'.question_id=webct.id AND current_chapter="'.$c.'" AND category IN ("PreLab0'.$c.'","Lab'.$c.'","Chapter'.$c.'") AND qtype IN ("MC","M")'; 
			//echo $query1.'<p>';
			//} else { 
			//	$query1 = 'SELECT question_id,qtype FROM '.$usertable.',webct WHERE '.$usertable.'.question_id=webct.id AND current_chapter="'.$c.'" AND category IN ("PreLab0'.$c.'","Lab'.$c.'","Chapter'.$c.'") AND qtype IN ("MC")';
			//}	
			
			//die($query1);
			$res1 = & $this->mdb2->query($query1);
			...
			while ($row1 = $res1->fetchAll()){
				...
				for($k=0;$k<count($row1);$k++) {
					$questionid = $row1[$k][0];
					$questiontype = $row1[$k][1];
					$query2 = "SELECT answered,comment FROM ".$usertable." WHERE question_id = ".$questionid;
					//echo $query2; //die();
					$res2 = & $this->mdb2->query($query2);
					$this->mdb2->disconnect();
					while ($row2 = $res2->fetchAll()){
						//var_dump($row2);
						for($l=0;$l<count($row2);$l++) {
							...
							$useranswer = $row2[$l][0];
							$config = $row2[$l][1];
							$scr = ($s->get_question_score($questionid,$useranswer,$config,$questiontype));
							//echo "<p>scr :" . $scr.'<p>';
							$score = $score + $s->get_total_score($scr,$useranswer,$questiontype);
							//echo "<p>score :" .'<p>'.$score;
						}
    ...
    $ch = $this->chapter; //6;
		echo 'ch: '.$ch;
		$chapter_score_arr = $this->computeChapterScores($ch);		
	...
	//=====================================================================//
		...
		$usertable = "stats_".$this->userid;
		//$query1 = "SELECT count(question_id) FROM ".$usertable." WHERE current_chapter='".$chapterNum."' AND deleted='0'";
		//$query1 = "SELECT count(question_id) FROM ".$usertable." WHERE current_chapter='".$chapterNum."' AND deleted='0'";
		
		$query1 = 'SELECT count(question_id) FROM '.$usertable.',webct WHERE '.$usertable.'.question_id=webct.id AND current_chapter="'.$chapterNum.'" AND category IN ("PreLab0'.$chapterNum.'","Lab'.$chapterNum.'","Chapter'.$chapterNum.'") AND qtype IN ("MC","M")';
		//die($query1);
		$res1 = & $mdb2->query($query1);
		$mdb2->disconnect();
		$row1 = $res1->fetchAll();
	...
	//=====================================================================//
  	...
  	//if ($this->status == 'admin') { 
  		$query1 = 'SELECT count(id) FROM webct WHERE category IN ("PreLab0'.$chapterNum.'","Lab'.$chapterNum.'","Chapter'.$chapterNum.'") AND qtype IN ("MC","M")'; 
  	//} else { 
  	//	$query1 = 'SELECT count(id) FROM webct WHERE category IN ("PreLab0'.$chapterNum.'","Lab'.$chapterNum.'","Chapter'.$chapterNum.'") AND qtype IN ("MC")';
  	//}
