      ...
      switch ($role) {
          ...
          default:
            for ($i = 0; $i < count($chArr); $i++) { 
            	$chList .= '<li><a href="#" class="chapter_index" id="chapter_index'.$chArr[$i].'" name="chapter" value="'.$chArr[$i].'">'.$chArr[$i].'</a></li>';
            }
          break;
      }
...
<div id="accor">
	<div class="accor">
		<h3 class="hel" id="headerQuestion" view="<?php echo $role=='student';?>"><a href="#" class="headering">Questions: <span name="Question" ch="<?php echo $chUser;?>"><?php echo $Header;?></span></a></h3>
		 <div class="ITS_ghost">		
				 <?php 
				 //echo $screen->main($mode);
				 if ($role == 'admin') { echo $screen->main($mode); }
				 else { 	
				    if ($status == 'BMED6787') { echo $screen->main($mode); } 
						else {
				   //$time = explode(':',date('G:i'));
				   //if ($time[0] < 3) { echo $screen->main(); }  //date("l") == 'Tuesday' AND 
				   //else {
					   echo '<ul class="ITS_footer" style="font-size:16pt"><li>ITS has closed.</li><li>"Practice" section will open shortly.</li><li>Your answers for Chapter 1-7, 9 & 11 are available for review.</li></ul>'; 
				   //}
					 }
				 }
				 ?>
	...
	<div class="accor">
		<h3 id="headerReview"><a href="#" class="header">Review: <span name="Review" qid="" ch="<?php echo $chUser;?>"><?php echo $Header;?></span></a></h3>
		<div class="ITS_ghost">
		<div id="reviewContainer">
			<?php 
			 $screen->screen = 4;
			 $screen->term_current = 'Spring_2011';
			 echo $screen->reviewMode(11,0);
			?>
			</div>
		</div>
	</div>	
	 ...
	 */
	 ...
	 $('#headerQuestion').live('click', function() {
	   ...
	   $('.chapter_index').each(function(index) {
				...
				if (v) { if ( (index) < chhide ){$(this).css({display: 'none'});} }
				if ($(this).text()==ch){$(this).css({background: '#FFFFFF',border: '2px solid #666666'});}
				else                   {$(this).css({background: 'gainsboro',border: '2px solid #aaaaaa'});}
	   })
	 })
	 /*-------------------------------------------------------------------------*/
	 ...
	 /*-------------------------------------------------------------------------*/
        ...
        $('.chapter_index').each(function(index) {
				  // update style
					if ( $(this).text() == current ){$(this).css({background: '#FFFFFF',  border: '2px solid #666666'});}
					else                            {$(this).css({background: 'gainsboro',border: '2px solid #AAAAAA'});}
				})
				// Which tab is opened?
          ...
          if (current == 'Survey') { 
  				...
  				$.get('ITS_screen_AJAX2.php', { ajax_args: callback, ajax_data: current.toLowerCase()}, function(data) {
...
//-------------- other -------------------//
    $(function() {
      ...
      $("#accor").accordion({autoHeight: false,navigation: true,collapsible:true,fillSpace:false,active: 0}); //,active: 1
