   ...
   case 'BMED6787': 
	 ...
	 default: 
      ...
      if ($role == 'admin') {
        ...
        /*switch ($status) {
         case 'BMED6787': $chMaxbreak;
      	 */
        $chMax = $chUser;  
      	$chArr = array(1,2,3,4,5,6,7,9,11);
      }
      /* -- */
...
switch ($status) {
   ...
   case 'BMED6787':
	 ...
	 /*-----------------*/
	    $mode   = 'question';
			$Header = 'Chapter '.$chUser;
	    $chList = '<span id="chText">CHAPTER</span><ul id="chList">';
      switch ($role) {
          case 'admin':
            for ($i = 1; $i <= $chMax; $i++) {
              $chList .= '<li><a href="#" class="chapter_index" name="chapter" value="'.$i.'">'.$i.'</a></li>';
            }
          break;
          default:
            for ($i = 0; $i < count($chArr); $i++) { 
            	$chList .= '<li><a href="#" class="chapter_index" name="chapter" value="'.$chArr[$i].'">'.$chArr[$i].'</a></li>';
            }
          break;
      }
...
style="float:right;margin-right:50px"
 navListActive
<li id="headerPractice"><a href="#" id="current">Practice</a></li> 
*/
//----------------//
?>
<div id="navContainer">
...
<li id="Practice" name="header" view="<?php echo $role=='student';?>" ch="<?php echo $chUser;?>"><a href="#">Practice</a></li>
<li id="Question" name="header" view="<?php echo $role=='student';?>" ch="<?php echo $chUser;?>"><a href="#" id="current">Questions</a></li>
<li id="Review"   name="header" view="<?php echo $role=='student';?>" ch="<?php echo $chUser;?>" style="float:right"><a href="#">Review</a></li>
</ul>
</div>
<?php 
...
if ($status == 'admin') {
	...
	if ($role == 'admin') {
		...
		'<li> ' . $spacer . ' <a href="survey1.php" style="color:#666">Spring 2011 Survey</a></li> ' .
	 ...
	 */
	 ...
	 $('#Practice').live('click', function() {
	   var v = $(this).attr('view');
		 ...
		 var current = $('.chapter_index#current').text();//$(this).text().replace(/^\s+|\s+$/g,""); // need to trim white spaces
		 //alert('PRA: '+current);
	   ...
	   $('.chapter_index').each(function(index) {
		 	...
		 	$.get('ITS_screen_AJAX2.php', { ajax_args: "practiceMode", ajax_data: current }, function(data) {
	 ...
	 $('#Question').live('click', function() {
	   ...
	   $('.chapter_index').each(function(index) {
		 ...
		 $.get('ITS_screen_AJAX2.php', { ajax_args: "newChapter", ajax_data: current+',question' }, function(data) {
				  $('#contentContainer').html(data);
					var qid = $('#ratingContainer').attr('qid');
					ratingUPDATE(qid);	// update rating module (style, rendering & state)								
		 })
	 ...
	 /*-------------------------------------------------------------------------*/
        ...
        $('.chapter_index').each(function(index) {
				  // update style
					if ($(this).text()==current){$(this).attr('id','current');}
				  else                        {$(this).attr('id','');}
				})
         ...
         //----------//
				 //alert(current);
					...
					var header   = 'Chapter '+current; 
  				$.get('ITS_screen_AJAX2.php', { ajax_args: callback, ajax_data: current+','+mode.toLowerCase()}, function(data) {
              $('#contentContainer').html(data);			
          })
         break;				
         //----------//
         case 'Question':
         //----------//
        	if (current == 'Survey') { var callback = "getSurvey";  var header = current; }
					else 				 	 					 { var callback = "newChapter"; var header = 'Chapter '+current; }		
  				$.get('ITS_screen_AJAX2.php', { ajax_args: callback, ajax_data: current.toLowerCase()+','+mode.toLowerCase()}, function(data) {
  						$('#accor div h3 a span[name="Question"]').text(header);
         ...
         //----------//
				    if (current == 'Survey') { 
						var header   = current; 
  				  $.get('ITS_screen_AJAX2.php', { ajax_args: "surveyMode", ajax_data: current+',0' }, function(data) {
  							$('#contentContainer').html(data);
    				    //alert($('.accordion .head').text());
    						//$('#accor div h3 a span[name=Review]').text(header);     
  						  //$('#accor div h3 a span[name=Review]').attr('ch',current);
            })
					}
          else {				 	 					 
					  var callback = "reviewMode"; 
						var header = 'Chapter '+current;
  				  $.get('ITS_screen_AJAX2.php', { ajax_args: callback, ajax_data: current+',0' }, function(data) {
  							$('#contentContainer').html(data);
    				    //alert($('.accordion .head').text());
    						//$('#accor div h3 a span[name=Review]').text(header);     
  						  //$('#accor div h3 a span[name=Review]').attr('ch',current);
	 ...
	 $('#ITS_next').live('click', function(event) {
	    var mode  = $(this).attr("mode");  //
			//alert(mode);
      $.get('ITS_screen_AJAX2.php', { ajax_args: "getContent", ajax_data: mode}, function(data) {
        $('#contentContainer').html(data);  
      })
	 })
	 /*-------------------------------------------------------------------------*/
	 $('.ITS_navigation[name=updateReview_index]').live('click', function(event) {
	    ...
	    var del = $(this).attr("del"); //alert(ch+' - '+del);		
			if (ch == 'Survey') {
        $.get('ITS_screen_AJAX2.php', { ajax_args: "surveyMode", ajax_data: ch+','+del}, function(data) {
  				$('#contentContainer').html(data);			
        })			
			}
			else {
	 ...
	 $('#ITS_submit').live('click', function() {
        ...
        var qtype  = $(this).attr("qtype");
         ...
         //----------//
            ...
            switch (mode) {
							case 'survey':  
								$.get('ITS_screen_AJAX2.php', { ajax_args: "recordSurveyAnswer", ajax_data: qid+'~'+qtype+'~'+chosen+'~'+c+'~'+ch+'~'+t+'~'+mode}, function(data) {
    						  $('#contentContainer').html(data);
                })
							break;
							default:
							  $.get('ITS_screen_AJAX2.php', { ajax_args: "recordChapterAnswer", ajax_data: qid+'~'+qtype+'~'+chosen+'~'+c+'~'+ch+'~'+t+'~'+mode}, function(data) {
