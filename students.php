...
echo '<p><span class="TextMessage" style="text-decoration:blink" id="Students_Message">' . $msg_ini . '</span><p>' . '<FORM id="add_class_Form" action="students.php" METHOD="POST">' . '<INPUT TYPE="file" name="new_class">&nbsp;&nbsp;' . '<INPUT TYPE="BUTTON" onclick="ITS_form_submit(this.name,"' . $msg . '")" VALUE="Add Class" NAME="add_class_Form">' . '</form>';
...
if (0) { //($datagrid->getRecordCount()>0){			
    ...
    $tb_course = '<table class="ITS_course">' . '<tr><th>NAME</td><th>TERM</td><th>YEAR</td><th></th></tr>';
    for ($i = 0; $i < count($course); $i++) {
        ...
        $del  = '<form id="delete_class_Form" action=students.php method="POST">' . '<INPUT TYPE=HIDDEN name="delete class" VALUE="' . $course[$i][1] . '">' . '<INPUT class=ITS_button TYPE=SUBMIT ONCLICK=ITS_form_submit(this.name,"' . $msg . '") NAME=delete_class_Form VALUE="delete">' . '</form>';
...
if (isset($_POST['new_class'])) {
    ...
    */
    $query = 'INSERT IGNORE INTO course (name,term,year,filename) VALUES (' . '"' . $fname . '",' . '"' . $term . '",' . $year . ',' . '"' . $filename . '")';
    //echo $query;die();
    $res =& $mdb2->exec($query);
    ...
    while (($data = fgetcsv($handle, 300, ",")) !== FALSE) {
        ...
        */
        ...
        if (!empty($res_u[0])) {
            ...
            $query      = 'INSERT INTO users (id,first_name,last_name,username,password,status,section) VALUES (' . $student_id . ', ' . '"' . $firstname . '", ' . '"' . $lastname . '", ' . '"' . $data[1] . '", ' . '"' . $data[1] . '", ' . '"' . $status . '", ' . $section . ')';
