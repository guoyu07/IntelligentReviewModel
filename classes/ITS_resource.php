...
class ITS_resource
{
    ...
    public function renderContainer()
    {
        ...
        for ($n = 0; $n < count($tharr); $n++) {
            ...
            $th .= '<td id="ITS_resource_'.strtolower($tharr[$n]).'"></td>';
        }
    ...
    public function getResource($field)
    {
        ...
        switch ($field) {
            //+++++++++++++++++++++++++++++++++++++++++++//
            case 'text':
                $query = 'SELECT content FROM dSPFirst WHERE content REGEXP "'.$this->concept.'" AND meta="paragraph"';
                break;
            case 'equation':
                //SELECT content FROM dSPFirst WHERE  name REGEXP "sampling" AND meta="equation";
                $query = 'SELECT code FROM concepts WHERE  name REGEXP "'.$this->concept.'"';
                break;
            case 'image':
            ...
            switch ($field) {
                ...
                case 'text':
                    ...
                    for ($i = 0; $i < count($li_arr); $i++) {
                        $tList .= '<tr><td><span class="ITS_List">' . $li_arr[$i] . '</span></td><td><input id="aa" type="button" class="resourceSelect" value="select"></td></tr>';
                    }
                    $tList .= '</table>';
                    break;
                default:
                    ...
                    for ($i = 0; $i < count($li_arr); $i++) {
                        ...
                        $tList .= '<div class="fl"><table class="tt"><tr><td><div class="ITS_latex">' . $li_arr[$i] . '</div></td></tr><tr><td><input type="button" class="resourceSelect" value="select"></td></tr></table></div>';
                    }
                    //$tList .= '</ul>';
            }
        }
