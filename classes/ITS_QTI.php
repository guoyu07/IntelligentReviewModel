...
/*=====================================================================/
  ...
  Revision: Greg Krudysz | Jul-11-2012
/=====================================================================*/
...
class ITS_QTI {
    ...
    //=====================================================================//
        global $db_dsn, $db_name, $tb_name, $tb_images,$db_table_user_state, $tex_path, $QTI_path;
        ...
        $this->tb_images  = $tb_images;
        $this->QTI_path = $QTI_path;
    }
    /*--------------------------------------------------------------------
    ...
    //====================================================================//	
        $linkID = mysql_connect($this->db_host, $this->db_user, $this->db_pass) or die("Could not connect to host.");
        mysql_select_db($this->db_name, $linkID) or die("Could not find database."); 
        ...
        //die('<p>'.$query.'</p>');
        $fields_arr = mysql_query($query, $linkID) or die("ITS_QTI.php: Data not found");
        $date_time  = $this->generateXMLContent($fields_arr, $table, $linkID);
        
        if ($cardinality == 1) {
            $str = '/'.$this->QTI_path.'/zipped_' . $date_time . '.zip';
        } else {			
            //$script = '<script>down();function down(){var url="'.$this->QTI_path.'/zipped_' . $date_time . '.zip";  
			//		window.open(url,"Download");}</script>';			
			$script = '<a href="'.$this->QTI_path.'/zipped_' . $date_time . '.zip" class="ITS_question_img">QTI files (zip)</a>';		
            $str    = '<div class="QTI_output">QTI files based on category : ' . $category  .'<p>' . $script . '<p></div>';
        }               
        return $str;
    }
    //====================================================================//
    ...
    //====================================================================//	
        ...
        for ($i = 0; $i < $count_ques; $i++) {
            ...
            if ($qtype[$i] == "mc" || $qtype[$i] == "MC") {
                ...
                /*------ QUESTION -------------------------------------------------------------*/
                $QTI .= '<material>' . "\n\t" . '<mattext texttype="text/html"><![CDATA[' . $ques[$i] . ']]></mattext>' . "\n\t" . $this->get_image_str($image[$i], $id[$i]) . '<mattext texttype="text/html"></mattext>' . '</material>' . "\n\t";
                ...
                $query      = 'SELECT ' . $ans_str . ',' . $weight_str . ',' . $image_str . ' FROM ' . $table . '_' . strtolower($qtype[$i]) . ' WHERE '.$table.'_id=' . $id[$i];
				//die('<p>'.$query.'</p>');              
                $ans_arr    = array();
                ...
                $query_m      = 'SELECT ' . $left_str . ',' . $leftimg_str . ',' . $right_str . ',' . $rightimg_str . ' FROM ' . $table . '_' . strtolower($qtype[$i]) . ' WHERE '.$table.'_id=' . $id[$i];
				//die('<p>'.$query_m.'</p>');                
                $response_arr = array();
                ...
                //$iden = $id[$i] + 1; ### GREG: why +1? ###
                $iden = $id[$i];
                for ($n = 0; $n < $anss[$i]; $n++) {
                    $QTI .= '<response_lid ident="QUE_' . $iden . '_RL">' . "\n" . '<material>' . "\n" . '<mattext texttype="text/html"><![CDATA[' . $left[$n] . ']]></mattext>' . "\n" . $this->get_image_str($left_img[$n], $id[$i]) . '</material>' . "\n" . '<render_choice shuffle="Yes">' . "\n";
                    
                    for ($m = 0; $m < $anss[$i]; $m++) {
                        $ans_ident = $id[$i] + $m + 2;
                        $num       = $m + 1;
                        // ### $ans_ident -> $iden
                        $QTI .= '<response_label ident="QUE_' . $iden . '_A' . $num . '">' . "\n" . '<material>' . "\n" . '<mattext texttype="text/html"><![CDATA[' . $right[$m] . ']]></mattext>' . "\n" . $this->get_image_str($right_img[$m], $id[$i]) . '</material>' . "\n" . '</response_label>' . "\n";
                    }
                ...
                $query   = 'SELECT * FROM ' . $table . '_' . strtolower($qtype[$i]) . ' WHERE '.$table.'_id=' . $id[$i];
        ...
        $this->generateXML($QTI);

        // echo '<br> adding to zip <br>';
        $cur_dir = getcwd();
        chdir($this->QTI_path);
        
		// echo '<p>1</p>'; echo getcwd(); die();
        $date_time = exec('date \'+%Y%m%d%H%M\'');
        $exec = 'zip -r zipped_' . $date_time . '.zip content/*';
        //echo '<p>'.$exec;
        exec($exec,$ret_zip);
        chdir($cur_dir);
        
        /* //die('<p>xxxx</p>');
        if (!copy('zipped_' . $date_time . '.zip', 'QTI/zipped_' . $date_time . '.zip')) {
            exec('rm -rf '.$this->QTI_path.'/content');
            die("<br>".$_SERVER["SCRIPT_NAME"].": failed to copy zipped file...<br>");
        }*/
        exec('rm -rf '.$this->QTI_path.'/content/*');
	...
	//====================================================================//
	//echo '<p>'.$this->QTI_path.'/content';die('===');		
        if (!file_exists($this->QTI_path.'/content')) {
            $old = umask(0);
            mkdir($this->QTI_path.'/content', 0777, true);
            umask($old);
        }

        // Output to file
        $f = fopen($this->QTI_path.'/content/allQues.xml', "w");
        fwrite($f, $QTI);
        fclose($f);
        //echo 'file wriiten';
    } // End of generateXML
	...
	//====================================================================//	
        if ($image != '') {
            if (!file_exists($this->QTI_path.'/content/media/' . $qid)) {
                $old = umask(0);
                mkdir($this->QTI_path.'/content/media/' . $qid, 0777, true);
                umask($old);
            }
            if (!copy('ITS_FILES/' . $image, $this->QTI_path.'/content/media/' . $qid . '/' . $name))
                echo ('ITS_QTI.php: failed with '.$image.' and '.$this->QTI_path.'/content/media/' . $qid . '/' . $name);
        }
    }
    //====================================================================//
    function get_image_str($iid, $qid){
    //====================================================================//	
		// $iid <- images.id
        // echo '<br color="red">'.$iid.' - '.$qid.'<br>';
        //die((empty($iid) || (strstr($iid, ".php") != false)));
        /*------ IMAGE ---------------------------------------------------------------*/
        if (empty($iid) || (strstr($iid, ".php") != false)) {
            $QTI_image = '';
        } else {
			$linkID = mysql_connect($this->db_host, $this->db_user, $this->db_pass) or die("Could not connect to host.");
			mysql_select_db($this->db_name, $linkID) or die("ITS_QTI.php: Could not find database.");
			$query  = "SELECT name,dir FROM " . $this->tb_images . " WHERE id=".$iid;
			//echo $query.'<br>';
			$img_arr = mysql_query($query, $linkID) or die("ITS_QTI.php: Data 1not found.");
            $row     = mysql_fetch_assoc($img_arr);
            $img     = $row['dir'] .'/'. $row['name'];		
            ...
            echo '<br>'.$img.'<br>';
            echo $qid.'<br>';
            echo $image_nm[$len_file_name].'<br>';
            echo '<br>'.$img.'<br>';
            die($QTI_image);*/
            
            $this->save_image($img, $qid, $image_nm[$len_file_name]);     
        }
    ...
    --------------------------------------------------------------------*/
    //====================================================================//
    function uploadFile($FILES){ /* function to upload a file */
        //====================================================================//	
        $obj         = new ITS_QTI();
            ...
            if ($FILES["file"]["error"] > 0) {
                ...
                foreach ($QTI_FILES as $QTIfile) {
                    ...
                    foreach ($question as $ques) {
                        ...
                        $query_question = "INSERT INTO ".$this->tb_name." (qtype,title, question, images_id, answers, category) VALUES ('" . $ques['qtype'] . "','" . $ques['qtitle'] . "', '" . $escaped_ques . "', '" . $ques['qimg'] . "','" . $opt_count . "', '" . $ques['qcat'] . "')";
                        ...
                        if (!(empty($ques['qimg']))) {
                            ...
                            $query_update   = "UPDATE ".$this->tb_name." SET image='" . $new_image_path . "' WHERE id=" . $id;
                            mysql_query($query_update) or die("error in updating image path");
                        }
                        switch ($ques['qtype']) {
                            case 'MC':
                                $opt_count = count($ques['qoptions']);
                                $tblName   = $this->tb_name."_" . strtolower($ques['qtype']);
                            ...
                            case 'C':
                                ...
                                $q = '';
                                $q = "INSERT INTO " . $tblName . " (".$tb_name."_id, formula, val1, min_val1, max_val1, val2, min_val2, max_val2, val3, min_val3, max_val3, val4, min_val4, max_val4, val5, min_val5, max_val5, val6, min_val6, max_val6, val7, min_val7, max_val7, val8, min_val8, max_val8, val9, min_val9, max_val9, val10, min_val10, max_val10, vals) VALUES (";
                        ...
                        $output .= '<div class="ITS_QTI"><div style="float:left;background:#fff;border:1px solid #666;padding: 0.1em;">Question ID <a href="Question.php?qNum=' . $id . '">' . $id . '</a></div>' . $ques['qtext'] . '</div>';
                        //}
                    }
    ...
    //====================================================================//	
        /* function to copy the image into the fodler from where the DB will refer it to */
