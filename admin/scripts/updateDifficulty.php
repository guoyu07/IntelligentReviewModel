...
FUNCTION runDif2($connection,$DataTable,$QTable,$DifCol,$pA,$pAR,$pD,$pS,$Dbug){
	...
	FOREACH($types as $QType){
		...
		WHILE ($row = mysql_fetch_array($result)){
			print "<tr><td>&nbsp;{$QType}&nbsp;</td>";
			print "<td>&nbsp;{$row['name']}&nbsp;</td><td>";
			if(updateDif2($connection,$DataTable,$QTable,$DifCol,$pA,$pAR,$pD,$pS,$row['id'],$QType,$Dbug))
				$total=$total+printDif($connection,$QTable,$DifCol,$row['id'],$QType,$Dbug);
			print "</td></tr>";
		}
...
FUNCTION runDifSTD($connection,$DataTable,$QTable,$DifCol,$pA,$pAR,$pD,$pS,$Dbug){
	...
	FOREACH($types as $QType){
		...
		WHILE ($row = mysql_fetch_array($result)){
			print "<tr><td>&nbsp;{$QType}&nbsp;</td>";
			print "<td>&nbsp;{$row['name']}&nbsp;</td><td>";
			if(updateDifSTD($connection,$DataTable,$QTable,$DifCol,$pA,$pAR,$pD,$pS,$row['id'],$QType,$Dbug))
				$total=$total+printDif($connection,$QTable,$DifCol,$row['id'],$QType,$Dbug);
			print "</td></tr>";
		}
...
FUNCTION runDifN($connection,$DataTable,$QTable,$DifCol,$pA,$pAR,$pD,$pS,$Drop,$Dbug){
	...
	FOREACH($types as $QType){
		...
		WHILE ($row = mysql_fetch_array($result)){
			//print "<tr><td>&nbsp;{$QType}&nbsp;</td>";
			//print "<td>&nbsp;{$row['name']}&nbsp;</td><td>";
			if(updateDifN($connection,$DataTable,$QTable,$DifCol,$pA,$pAR,$pD,$pS,$row['id'],$QType,$Drop,$Dbug))
				$total=$total+printDif($connection,$QTable,$DifCol,$row['id'],$QType,$Dbug);
			//print "</td></tr>";
		}
...
FUNCTION updateDifN($connection,$DataTable,$QTable,$DifCol,$pA,$pAR,$pD,$pS,$tag_id,$QType,$Drop,$Dbug){
	...
	if(!($result=MyQuery($query,$connection,"query for mean and std failed",1)))
		$ran=0;
	...
	$queryDMax = "SELECT AvgDur as Dmax FROM {$DataTable} JOIN questions ON questions.id={$DataTable}.question_id 
				WHERE {$DataTable}.question_id=questions.id 
				AND questions.qtype LIKE '{$QType}' Order by AvgDur DESC LIMIT {$Drop},1;";
	...
	$queryDMin = "SELECT AvgDur as Dmin FROM {$DataTable} JOIN questions ON questions.id={$DataTable}.question_id 
				WHERE {$DataTable}.question_id=questions.id 
				AND questions.qtype LIKE '{$QType}' Order by AvgDur LIMIT {$Drop},1;";
	//print($queryDMin);
	...
	$querySMax = "SELECT NumSkips as Smax FROM {$DataTable} JOIN questions ON questions.id={$DataTable}.question_id 
				WHERE {$DataTable}.question_id=questions.id 
				AND questions.qtype LIKE '{$QType}' Order by NumSkips DESC LIMIT {$Drop},1;";
	...
	$querySMax = "SELECT NumSkips as Smin FROM {$DataTable} JOIN questions ON questions.id={$DataTable}.question_id 
				WHERE {$DataTable}.question_id=questions.id 
				AND questions.qtype LIKE '{$QType}' Order by NumSkips LIMIT {$Drop},1;";
