...
class user_stats{
	...
	function GetDataDiffFilter($min_concepts,$diff_threshold){

		if($this->debug)echo "<center>GetDataWithFilter<br></center>";
        
        	$concepts = $this->GetConcepts();
        
        	if(sizeof($concepts)<=$min_concepts * 100/$diff_threshold)
			$numConcepts = $min_concepts;
		else
			$numConcepts = ceil($diff_threshold * sizeof($concepts));
